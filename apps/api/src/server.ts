...
export function createServer() {
  ...
  app.post('/api/v1/webhooks/billing/stripe', async (req: Request, res: Response) => {
    try {
      ...
      const eventId = req.body.id || req.body.event_id || `evt_stripe_${crypto.randomBytes(8).toString('hex')}`;
      ...
      const providerSubId = objectData.subscription || objectData.id || `sub_stripe_${crypto.randomBytes(8).toString('hex')}`;
      ...
      if (workspaceId) {
        await query(
          `INSERT INTO subscriptions (workspace_id, plan_id, provider, provider_subscription_id, status, current_period_start, current_period_end)
           ...
           ON CONFLICT (workspace_id)
           DO UPDATE SET plan_id = $2, provider = 'STRIPE', provider_subscription_id = $3, status = 'ACTIVE', current_period_start = CURRENT_TIMESTAMP, current_period_end = CURRENT_TIMESTAMP + INTERVAL '1 year'`,
          [workspaceId, planId, providerSubId]
        );
      }

      await query(
        `INSERT INTO billing_webhook_deliveries (provider, provider_event_id, event_type, payload, status)
         VALUES ('STRIPE', $1, $2, $3, 'PROCESSED')`,
