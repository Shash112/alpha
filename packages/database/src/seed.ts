...
export async function seedDatabase(): Promise<void> {
  ...
  for (const perm of permissions) {
    await query(
      ...
      [perm.id, perm.category, perm.description]
    );
  }
  ...
  for (const plan of plans) {
    await query(
      `INSERT INTO plans (id, family, name, billing_period, price_inr, entitlements_schema)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (id) DO UPDATE SET price_inr = $5, entitlements_schema = $6`,
      [plan.id, plan.family, plan.name, plan.billing_period, plan.price_inr, JSON.stringify(plan.entitlements)]
    );
  }
  ...
  for (const tmpl of templates) {
    await query(
      ...
      [tmpl.id, tmpl.name, tmpl.slug, tmpl.category, tmpl.layout_schema]
    );
  }
