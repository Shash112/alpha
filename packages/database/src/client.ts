...
function isConnRefused(error: any): boolean {
  ...
  if (typeof error.message === 'string' && error.message.includes('ECONNREFUSED')) return true;
  if (Array.isArray(error.errors)) {
    return error.errors.some((e: any) => e?.code === 'ECONNREFUSED' || (typeof e?.message === 'string' && e.message.includes('ECONNREFUSED')));
  }
  return false;
}
...
let memInitPromise: Promise<void> | null = null;

function cleanQueryForPgMem(text: string): string {
...
async function executeMemQuery(mem: any, text: string, params?: any[]): Promise<any> {
  if (memInitPromise) {
...
function getMemDbPool(): any {
  if (memPoolInstance) return memPoolInstance;
  ...
  rawPool.query = function (text: any, params?: any): any {
  ...
  rawPool.connect = async function (): Promise<any> {
    const client = await originalConnect();
    const originalClientQuery = client.query.bind(client);
    client.query = function (text: any, params?: any): any {
      if (typeof text === 'string' && text.toUpperCase().includes('CREATE EXTENSION')) {
        const clean = cleanQueryForPgMem(text);
        if (!clean.trim()) {
          return Promise.resolve({ rows: [], rowCount: 0, command: '', oid: 0, fields: [] });
...
async function initMemDbSchema(memPool: any) {
  try {
    const ddl = `
    ...
    for (const stmt of ddl.split(';')) {
      ...
      if (trimmed) await memPool.query(trimmed);
    }
